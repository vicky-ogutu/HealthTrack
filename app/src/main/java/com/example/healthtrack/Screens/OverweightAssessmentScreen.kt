...
fun OverweightAssessmentScreen(
    ...
    var drugsExpanded by remember { mutableStateOf(false) }
    ...
    Scaffold(
        topBar = {
            TopAppBar(
                title = { Text("Overweight Assessment") }
            )
        },
        bottomBar = {
            BottomAppBar {
        ...
        Column(
            ...
            Box(modifier = Modifier.fillMaxWidth()) {
                OutlinedTextField(
                    value = onDrugs,
                    onValueChange = { },
                    label = { Text("Are you currently using any drugs? *") },
                    readOnly = true,
                    modifier = Modifier.fillMaxWidth(),
                    trailingIcon = {
                        IconButton(onClick = { drugsExpanded = true }) {
                            Icon(Icons.Default.ArrowDropDown, contentDescription = "Select drugs")
                        }
                    }
                )
                ...
                DropdownMenu(
                    ...
                    onDismissRequest = { drugsExpanded = false }
                ) {
                    drugsOptions.forEach { option ->
                        DropdownMenuItem(
                            text = { Text(option) },
                            onClick = {
                                viewModel.updateOnDrugs(option)
                                drugsExpanded = false
                            }
                        )
                    }
